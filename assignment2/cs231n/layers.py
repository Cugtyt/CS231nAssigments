...
def batchnorm_backward_alt(dout, cache):
    ...
    dvar = np.sum(
        ...
        axis=0)
    dmean = np.sum(
        -dx_hat / np.sqrt(sample_var + eps), axis=0) + dvar * np.mean(
            -2 * (x - sample_mean), axis=0)
    dx = 1 / np.sqrt(sample_var + eps) * dx_hat + dvar * 2 / N * (
        x - sample_mean) + 1 / N * dmean
    dgamma = np.sum(x_hat * dout, axis=0)
    dbeta = np.sum(dout, axis=0)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ...
    for i in range(H_out):
        for j in range(W_out):
            ...
            dx[:, :, i * stride:i * stride + HH, j * stride:j * stride +
               WW] += temp_binary_mask * dout[:, :, i, j][:, :, None, None]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
